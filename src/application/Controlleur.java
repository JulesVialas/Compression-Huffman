...
public class Controlleur {
    ...
    private void compterOccurrences() {
        File fichier = choisirFichier("Sélectionner un fichier texte (.txt)",
                "*.txt", false);
        if (fichier != null) {
            try {
        ...
        if (fichier != null) {
            try {
                File fichierEnregistrement = choisirFichier(
                        "Enregistrer l'arbre Huffman", "*.txt", true);
        ...
        if (fichier != null) {
            try {
                File dictionnaire = choisirFichier(
                        "Sélectionner un dictionnaire Huffman (.txt)", "*.txt",
                        false);
                if (dictionnaire != null) {
                    File fichierEnregistrement = choisirFichier(
                            "Enregistrer le fichier compressé (.bin)", "*.bin",
                            true);
                    if (fichierEnregistrement != null) {
    ...
    private void decompresserFichier() {
        ...
        if (fichier != null) {
            try {
                ...
                if (dictionnaire != null) {
                    File fichierEnregistrement = choisirFichier(
                            "Enregistrer le fichier décompressé", "*.txt",
                            true);
                    if (fichierEnregistrement != null) {
                        long startTime = System.nanoTime();
    ...
    private void afficherInfosCompression(File originalFile,
        ...
        alert.setContentText(String.format(
                ...
                originalFile.length(), compressedFile.length(), duration,
                tauxCompression));
        alert.showAndWait();
    }
